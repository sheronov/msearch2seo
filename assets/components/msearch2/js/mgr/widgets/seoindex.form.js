mSearch2.panel.Index = function (config) {
    config = config || {};
    Ext.applyIf(config, {
        id: 'modx-panel-search'
        , cls: 'container form-with-labels mse2-index-form'
        , labelAlign: 'left'
        , autoHeight: true
        , title: _('search_criteria')
        , labelWidth: 200
        , url: mSearch2.config.connector_url
        , baseParams: {
            action: 'mgr/index/createseo'
        }
        , items: [{
            layout: 'column',
            border: false,
            anchor: '100%',
            items: [{
                columnWidth: .4
                , layout: 'form'
                , cls: 'main-wrapper'
                , border: false
                , items: this.getFields(config, false)
            }, {
                columnWidth: .6
                , layout: 'form'
                , cls: 'main-seo-wrapper'
                , border: false
                , items: this.getFields(config, true)
            }]
        }, {
            html: '<hr />'
            , border: false
        }]
        , buttonAlign: 'left'
        , buttons: [{
            text: '<i class="icon icon-refresh"></i> ' + _('mse2_index_create')
            , handler: function () {
                this.submit(this);
                //this.indexCreate(0);
            }, scope: this
        }
            , '-'
            , {
                text: '<i class="icon icon-trash-o"></i> ' + _('mse2_index_clear')
                , handler: function () {
                    this.indexClear();
                }, scope: this
            }]
        , listeners: {
            render: {fn: this.getStat, scope: this}
            , success: {
                fn: function (response) {
                    var data = response.result.object;
                    var form = this.getForm();
                    var values = form.getValues();
                    if (!data.done) {
                        form.setValues({
                            offset: Number(data.offset)
                        });
                        this.submit(this);
                    } else {
                        this.getStat();
                    }
                }, scope: this
            }
        }
    });
    mSearch2.panel.Index.superclass.constructor.call(this, config);
};

Ext.extend(mSearch2.panel.Index, MODx.FormPanel, {
    filters: {}

    , getFields: function (config, seo = false) {
        var fields = [];
        var tmp = {
            total: {value: 0, disabled: true}
            , total_seo: {value: 0, disabled: true, seo: true, label:'Всего SEO страниц'}
            , total_seo_results: {value: 0, disabled: true, seo: true, label:'Из них с результатами'}
            , indexed: {value: 0, disabled: true}
            , indexed_seo: {value: 0, disabled: true, seo: true, label:'SEO страниц в индексе'}
            , words: {value: 0, disabled: true}
            , words_seo: {value: 0, disabled: true, seo: true, label:'Проиндесировано SEO слов'}
            , delimeter: {xtype: 'displayfield', value: '&nbsp;'}
            , limit: {value: 100, minValue: 1, maxValue: 1000}
            , offset: {value: 0}
        };

        for (var i in tmp) {
            if (!tmp.hasOwnProperty(i)) {
                continue;
            }
            var field = tmp[i];
			if (seo && !field.hasOwnProperty('seo')) {
				continue;
			}
			if (!seo && field.hasOwnProperty('seo')) {
				continue;
			}
            Ext.applyIf(field, {
                name: i
                , xtype: 'numberfield'
                , width: 60
                , fieldLabel: _('mse2_index_' + i) || field.label
                , allowDecimals: false
                , allowNegative: false
            });
            fields.push(field);
        }

        return fields;
    }
    /*
        ,indexCreate: function(offset) {
            var el = this.getEl();
            el.mask(_('loading'),'x-mask-loading');
            MODx.Ajax.request({
                url: mSearch2.config.connector_url
                ,params: {
                    action: 'mgr/index/create'
                    ,limit: 10
                    ,offset: offset
                }
                ,listeners: {
                    success: {fn:function(r) {
                        if (r.object.indexed > 0) {
                            this.indexCreate(Number(r.object.indexed) + Number(offset));
                        }
                        else {
                            el.unmask();
                            this.getStat();
                        }
                    },scope: this}
                    ,failure: {fn:function(r) {
                        el.unmask();
                    }, scope:this}
                }
            })
        }
    */
    , indexClear: function () {
        var el = this.getEl();
        el.mask(_('loading'), 'x-mask-loading');
        MODx.Ajax.request({
            url: mSearch2.config.connector_url
            , params: {
                action: 'mgr/index/remove'
            }
            , listeners: {
                success: {
                    fn: function (r) {
                        el.unmask();
                        this.getStat();
                    }, scope: this
                }
                , failure: {
                    fn: function (r) {
                        el.unmask();
                    }, scope: this
                }
            }
        })
    }

    , getStat: function () {
        var el = this.getEl();
        el.mask(_('loading'), 'x-mask-loading');
        MODx.Ajax.request({
            url: mSearch2.config.connector_url
            , params: {
                action: 'mgr/index/statseo'
            }
            , listeners: {
                success: {
                    fn: function (r) {
                        el.unmask();
                        var form = this.getForm();
                        r.object.offset = 0;
                        form.setValues(r.object);
                    }, scope: this
                }
                , failure: {
                    fn: function (r) {
                        el.unmask();
                    }, scope: this
                }
            }
        })
    }

});
Ext.reg('msearch2-form-index', mSearch2.panel.Index);